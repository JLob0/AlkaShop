...
- **Venda automática (auto-sell)**, restrita a jogadores com a permissão
  `alkashop.autosell` (pensado pra VIPs) — pode ser ligada/desligada por comando
  (`/venderautomatico`) ou forçada.
- **Vender tudo / vender item na mão** (`/vendertudo`, `/vendermao`) pra fluxo
  rápido sem abrir GUI.
- **API pública** (`AlkaShopAPI`) publicada via `ServicesManager`, consumida por
  ...
  `isAutoSellActive(player)` e `sellItem(...)` permitem que um bloco minerado seja
  vendido automaticamente sem esse outro plugin nunca precisar saber o preço de
  nada, só perguntar "devo vender isso pra este jogador?".
